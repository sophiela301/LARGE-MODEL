...
class GroupDataset(object):
    def __init__(self, user_path, group_path, num_negatives, dataset="Mafengwo"):
        ...
        if dataset == "MafengwoS":
            ...
            self.user_train_matrix = load_rating_file_to_matrix(
                user_path + "Train.txt")
        self.user_test_ratings = load_rating_file_to_list(
            user_path + "Test.txt")
        self.user_test_negatives = load_negative_file(
            user_path + "Negative.txt")
        ...
        self.group_test_ratings = load_rating_file_to_list(
            group_path + "Test.txt")
        self.group_test_negatives = load_negative_file(
            group_path + "Negative.txt")
        ...
        # Item-level graph
        self.light_gcn_graph = build_light_gcn_graph(
            self.group_train_matrix, self.num_groups, self.num_group_net_items)
        ...
        for (u, i) in train.keys():
            for _ in range(self.num_negatives):
                ...
                while (u, j) in train:
                    j = np.random.randint(num_items)
                neg_items.append(j)
        pos_neg_items = [[pos_item, neg_item]
                         for pos_item, neg_item in zip(pos_items, neg_items)]
    ...
    def get_user_dataloader(self, batch_size):
        ...
        train_data = TensorDataset(torch.LongTensor(
            users), torch.LongTensor(pos_neg_items))
    ...
    def get_group_dataloader(self, batch_size):
        ...
        train_data = TensorDataset(torch.LongTensor(
            groups), torch.LongTensor(pos_neg_items))
        return DataLoader(train_data, batch_size=batch_size, shuffle=True)
