...
def build_hyper_graph(group_member_dict, group_train_path, num_users, num_items, num_groups, group_item_dict=None):
    ...
    if group_item_dict is None:
        ...
        for line in open(group_train_path, 'r').readlines():
            contents = line.split()
            if len(contents) > 2:
                group, item, rating = int(contents[0]), int(
                    contents[1]), int(contents[2])
                if rating > 0:
    ...
    group_data = [group_member_dict[group_id] + group_item_dict[group_id]
                  for group_id in range(num_groups)]
    ...
    full_hyper_graph = torch.sparse.mm(convert_sp_mat_to_sp_tensor(
        hg_dg), convert_sp_mat_to_sp_tensor(full_hg))
    print(
...
def build_light_gcn_graph(group_item_net, num_groups, num_items):
    ...
    adj_mat = sp.dok_matrix(
        (num_groups + num_items, num_groups + num_items), dtype=np.float32)
